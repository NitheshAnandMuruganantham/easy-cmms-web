...
import NewSection from "./newInvoices";
...
function Invoices() {
  ...
  const [viewInvoice, setShowViewInvoiceModal] = useState<{
    open: boolean;
    rowId: number;
  }>({
    rowId: 0,
  ...
  const [DeleteSection] = useRemoveInvoicesMutation();
  useEffect(() => {
  ...
  return (
    <div>
      ...
      />
      <ViewInvoice
        rowId={viewInvoice.rowId}
        open={viewInvoice.open}
        close={() => {
          setShowViewInvoiceModal({
            open: false,
            rowId: viewInvoice.rowId,
          });
        }}
      ...
      <DataGrid
        ...
        autoHeight
        logLevel="debug"
        error={GetInvoicesError || MachineCountError}
        ...
        loading={GetInvoiceLoading || MachineCountLoading}
        ...
        rowCount={MachinesCount?.invoiceCount || 0}
        columns={[
          ...columns,
          {
            field: "view",
            ...
            renderCell: (params) => (
              <Button
                onClick={() => {
                  setShowViewInvoiceModal({
                    open: true,
                    rowId: parseInt(params.row.id, 10),
                  });
                  RefetchMaintenanceCount();
                  RefetchMaintenances();
                }}
                    ...
                    buttons: [
                      {
                        ...
                        onClick: async () => {
                          await DeleteSection({
                            variables: {
                              removeInvoicesId: parseInt(params.row.id, 10),
                            },
                          })
                            .then((res) => {
                              if (res.data?.removeInvoices) {
